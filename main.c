...
const static ec_pdo_entry_reg_t domain0_regs[] = {
    ...
    {SOLO_Slave00_Pos, SOLO_SLAVE00_ID, 0x603F, 0x00, &ec_slave0.error_code, NULL},
    {}
};
...
static void ec_slave_config_init(ec_slave_config_t *sc)
{
    ...
    ecrt_slave_config_sdo32(sc, 0x1600, 2, 0x607A0020); /*0x607A:0/32bits, Target postion*/
    ecrt_slave_config_sdo32(sc, 0x1600, 3, 0x60FF0020); /*0x60FF:0/32bits, Target velocity*/
    ecrt_slave_config_sdo32(sc, 0x1600, 4, 0x60710010); /*0x6071:0/32bits, Target torque*/
    ecrt_slave_config_sdo32(sc, 0x1600, 5, 0x60600008); /*0x6060:0/8bits, modes of operation*/
    ...
    ecrt_slave_config_sdo32(sc, 0x1A00, 2, 0x60640020); /*0x6064:0/32bits, Position actual value*/
    ecrt_slave_config_sdo32(sc, 0x1A00, 3, 0x606C0020); /*0x606C:0/32bits, velocity actual value*/
    ecrt_slave_config_sdo32(sc, 0x1A00, 4, 0x60770010); /*0x606C:0/16bits, torque actual value*/
    ecrt_slave_config_sdo32(sc, 0x1A00, 5, 0x60610008); /*0x6061:0/8bits, modes of operation display*/
    ecrt_slave_config_sdo32(sc, 0x1A00, 6, 0x603F0010); /*0x603F:0/16bits, error code value*/
    ecrt_slave_config_sdo8(sc, 0x1A00, 0, 6); /*set number of TxPDO*/
...
static void ec_readmotordata(Ctl_Data* ec_motor,Slave_Data ec_slave)
{
    ...
    ec_motor->errorCode = EC_READ_S16(domain_pd + ec_slave.error_code);

    // 小状态机
    if ((ec_motor->statusWord & 0xFF) == 0x40) // 取低8位
        ec_motor->controlWord = 0x6;
    else if ((ec_motor->statusWord & 0xFF) == 0x21){
        ec_motor->controlWord = 0x7;
        ec_motor->targetPos = ec_motor->actualPos;
    }else if ((ec_motor->statusWord & 0xFF)==0x23){
        ec_motor->controlWord = 0xf;
    }
    else if ((ec_motor->statusWord & 0xFF)==0x27)
...
void *my_thread(void *arg)
{
    ...
    while ((run != 0 )||(close_signal != 0)) {
        ...
        printf("count,%d, mode:%02x, CW,%04x, SW,%04x, pos,%08x, vel,%08x, target pos:%08x, torque,%x EC,%04x,\n", 
            cycle_counter,
            ec_motor0.actualMode,
            ec_motor0.controlWord,
            ec_motor0.statusWord,
            ec_motor0.actualPos,
            ec_motor0.actualVel,
            ec_motor0.targetPos,
